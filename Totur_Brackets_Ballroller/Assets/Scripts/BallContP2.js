...
//2D Jump
   if ((playerType == 0) && Input.GetButtonDown("2DJump_P2") && IsGrounded() )
	{
...
//Acceleration Controle
	//Ground
	if (Input.GetButton("Horizontal_P2") && (GetComponent.<Rigidbody>().velocity.x < maxSpeed) && (GetComponent.<Rigidbody>().velocity.x > minSpeed) && IsGrounded()) 
		{
//		Debug.Log("Accelerating on ground...");
		GetComponent.<Rigidbody>().velocity.x = GetComponent.<Rigidbody>().velocity.x + (acceleration * Input.GetAxis ("Horizontal_P2"));
		}
	//Air
	else if (Input.GetButton("Horizontal_P2") && (GetComponent.<Rigidbody>().velocity.x < maxAirSpeed) && (GetComponent.<Rigidbody>().velocity.x > minAirSpeed))
		{
//		Debug.Log("Accelerating in Air...");
		GetComponent.<Rigidbody>().velocity.x = GetComponent.<Rigidbody>().velocity.x + (airAcceleration * Input.GetAxis ("Horizontal_P2"));
		}
