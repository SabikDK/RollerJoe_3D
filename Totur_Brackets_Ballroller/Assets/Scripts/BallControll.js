...
//2D Jump
   if ((playerType == 0) && Input.GetButtonDown("2DJump") && IsGrounded() )
	{
...
//Acceleration Controle
	//Ground
	if (Input.GetButton("Horizontal") && (GetComponent.<Rigidbody>().velocity.x < maxSpeed) && (GetComponent.<Rigidbody>().velocity.x > minSpeed) && IsGrounded()) 
		{
//		Debug.Log("Accelerating on ground...");
	...
	//Air
	else if (Input.GetButton("Horizontal") && (GetComponent.<Rigidbody>().velocity.x < maxAirSpeed) && (GetComponent.<Rigidbody>().velocity.x > minAirSpeed))
		{
//		Debug.Log("Accelerating in Air...");
		GetComponent.<Rigidbody>().velocity.x = GetComponent.<Rigidbody>().velocity.x + (airAcceleration * Input.GetAxis ("Horizontal"));
		}
